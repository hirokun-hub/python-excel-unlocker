...
const getPresignedUrls = async (
  files: FileInfo[],
): Promise<{ items: PresignedUrlResponseItem[] }> => {
  if (USE_MOCK)
    return {
      items: files.map((f) => ({
        filename: f.filename,
        s3_key: `mock/${f.filename}`,
        upload_url: "mock://put",
      })),
    };
...
const unlock = async (
  files: UnlockFile[],
  passwords: string[],
): Promise<AxiosResponse<UnlockSyncResponse | UnlockAsyncResponse>> => {
  if (USE_MOCK) {
    ...
    return {
      status: 200,
      data: { results },
    } as unknown as AxiosResponse<UnlockSyncResponse>;
  }
  ...
  return (
    <header className="flex justify-between items-center">
      <div className="flex-grow">
        <CardTitle>Excel パスワード解除</CardTitle>
        <CardDescription>
          パスワード付きExcelファイルを一括で解除します
        </CardDescription>
      </div>
      {session && (
  ...
  <div className="absolute inset-0 bg-background/80 backdrop-blur-sm flex flex-col justify-center items-center z-10 rounded-lg">
    <div className="text-center p-6">
      ...
      <p className="text-muted-foreground mb-8">
        アプリケーションの利用には、Googleアカウントでのサインインが必要です。
      </p>
...
export default function Home() {
  ...
  const handleProcessClick = async () => {
    ...
    if (!pass1 && !pass2) {
      setError("少なくとも1つのパスワードを入力してください。");
      return;
    }
    setError(null);
    ...
    try {
      if (USE_MOCK) {
        ...
        if (passwords.length === 0) {
          throw new Error("少なくとも1つのパスワードを入力してください。");
        }
        ...
        for (const f of selectedFiles) {
          ...
          mockResults.push({
            fileName: f.name.replace(/\.xlsx?$/i, "_unlocked.xlsx"),
            status: "success",
            downloadUrl: url,
          });
          setJobProgress((prev) => ({
      ...
      const uploadPromises = selectedFiles.map((file) => {
        const presignedData = presignedItems.find(
          (p) => p.filename === file.name,
        );
        if (!presignedData)
          throw new Error(`${file.name}の署名付きURLが取得できませんでした。`);
      ...
      setProcessingStatus("processing");
      const unlockFiles = selectedFiles.map((f) => ({
        s3_key: presignedItems.find((p) => p.filename === f.name)!.s3_key,
      ...
      if (unlockResponse.status === 200) {
        ...
        toast.info(
          `処理を開始しました (Job ID: ${data.job_id})。完了までお待ちください...`,
        );
  ...
  const progressValue =
    processingStatus === "uploading" ? totalUploadProgress : jobPercentage;
  const progressText =
    processingStatus === "uploading"
      ? `ファイルをアップロード中... (${totalUploadProgress}%)`
      : `サーバーでファイルを処理中... (${jobPercentage}%)`;
        ...
        <Card className="w-full max-w-4xl relative">
          ...
          <div className={!isAuthenticated ? "blur-sm" : ""}>
              ...
              {processingStatus === "idle" && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Input
                      type="password"
                      placeholder="パスワード候補1"
                      value={pass1}
                      onChange={(e) => setPass1(e.target.value)}
                    />
                    <Input
                      type="password"
                      placeholder="パスワード候補2（任意）"
                      value={pass2}
            ...
            <CardFooter className="flex justify-center gap-4 pt-6">
              ...
              {processingStatus === "done" && (
                <Button size="lg" onClick={handleClearClick}>
                  もう一度試す
                </Button>
              )}
            </CardFooter>
