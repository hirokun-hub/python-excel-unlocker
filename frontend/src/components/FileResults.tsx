...
export function FileResults({ results }: { results: ProcessResult[] }) {
  ...
  const handleSave = async (file: ProcessResult) => {
    ...
    const toastId = toast.loading(
      `${file.fileName} をGoogle Driveに保存しています...`,
    );
    try {
      const blob = await fetch(file.downloadUrl).then((r) => r.blob());
      await uploadToDriveUsingAccessToken(
        blob,
        file.fileName,
        session.accessToken,
        { parentId: folder?.id },
      ...
      if (err instanceof DriveUploadError) {
        if (err.status === 401) {
          ...
          errorMessage =
            "Google Drive APIが未有効か、スコープが不足しています（drive.file）。";
        } else {
  ...
  const saveAll = async () => {
    ...
    const filesToSave = results.filter(
    ...
    <>
      ...
      <div className="space-y-3">
        {results.map((result, index) => (
          <div
            key={index}
            className={`flex justify-between items-center p-3 rounded-lg ${result.status === "success" ? "bg-green-100 dark:bg-green-900/20" : "bg-red-100 dark:bg-red-900/20"}`}
          >
            <span
              className={`font-medium ${result.status === "success" ? "text-green-800 dark:text-green-300" : "text-red-800 dark:text-red-300"}`}
            >
              {result.fileName}
            </span>
            {result.status === "success" ? (
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={savingToDrive.includes(result.fileName)}
                >
                  {savingToDrive.includes(result.fileName) ? (
                    <>
