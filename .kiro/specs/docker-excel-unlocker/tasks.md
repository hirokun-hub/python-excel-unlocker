...
`data/` ディレクトリにパスワード保護された Excel テストファイルを配置済み。

| ファイル | パスワード |
|---------|-----------|
| `data/確定【20251031】【SB】【NPS評価】...xlsx` | `test1234` |
| `data/確定【20251031】【SB】【クルー】...xlsx` | `test1234` |
| `data/確定【20251031】【YM】【クルー】...xlsx` | `test1234` |

**使用箇所:** タスク 4, 5, 11, 18, 19, 21 のテストで実データを使用
...
- [x] 12. Jinja2 テンプレートの作成
  - [x] 12.1 index.html の作成
    - ファイル選択（複数可）
    - 第1パスワード入力欄（必須）
    - 第2パスワード入力欄（任意、空欄可）
    - 解除ボタン
...
- [x] 13. CSS スタイルの作成
  - [x] 13.1 style.css の作成
    - シンプルで直感的なデザイン
    - モバイルファーストのレスポンシブ対応
    - 処理状態の視覚的フィードバック
    - _Requirements: 2.3, 2.4, 3.1, 3.3_

- [x] 14. JavaScript クライアントの実装
  - [x] 14.1 app.js の作成
...
- [ ] 18. 統合テストの作成
  - [ ]* 18.1 API 統合テストの作成
    ...
    - **テストデータ使用**: `data/` 内の Excel ファイルで E2E フローを検証
    - _Requirements: 全 API 仕様_

- [x] 19. パフォーマンステストスクリプトの作成
  - [x] 19.1 perf-smoke.sh の作成
    - 20MB ダミーファイルの生成（または `data/` 内の実ファイルを使用）
    - N=4 並列リクエストの送信
